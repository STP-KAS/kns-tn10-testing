#!/usr/bin/env node
/**
 * KNS TN10 single-domain create (commit + reveal) - Phase A copy with fast RPC connect.
...
// Phase-A copy of kns-smoke-create.mjs: identical tx logic; faster RPC connect
// (resolve URL first, connect directly; fall back to resolver-driven connect).
let rpc;
try {
  const url = await new kaspa.Resolver().getUrl(kaspa.Encoding.Borsh, network);
  rpc = new kaspa.RpcClient({ url, encoding: kaspa.Encoding.Borsh, networkId: network });
  await rpc.connect({ timeoutDuration: 10000 });
} catch (e) {
...
// Resume: if a previous (interrupted) attempt already committed to this label's P2SH, reveal from it
// instead of committing again (a pre-existing P2SH UTXO otherwise makes the reveal race its own new commit).
let p2shEntry = null;
let commitId = null;
{
  const { entries: pre } = await rpc.getUtxosByAddresses([p2shAddress]);
  if (pre && pre.length) {
    p2shEntry = pre[0];
    commitId = String(p2shEntry.outpoint?.transactionId ?? "") || null;
    console.log(JSON.stringify({ phase: "resume_existing_p2sh", count: pre.length, outpoint: p2shEntry.outpoint }));
  }
}
const { entries } = await rpc.getUtxosByAddresses([payer]);
if (!p2shEntry && !entries.length) {
...
} // end commit (skipped when resuming an existing P2SH)
console.log(
  JSON.stringify({
    phase: "p2sh_utxo",
